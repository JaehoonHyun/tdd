...
//MISSION: circular operator must be working well.
...
struct Producer2_1
{
	...
	uint64_t claim(Disruptor2_1 &dis)
	{

		uint64_t captureNext;
		captureNext = (dis.next.fetch_add(1) + 1) & (RING_BUFFER_SIZE - 1);

		//checking consumer when is not consuming
		//wait until capturePos bigger than captureNext
		do
		{
			uint64_t capturePos = (dis.pos + 1) & (RING_BUFFER_SIZE - 1);
			if (captureNext < capturePos)
			{
				break;
			}
	...
	uint64_t commit(Disruptor2_1 &dis, uint64_t idx)
	{
	...
	uint64_t push(Disruptor2_1 &dis, uint64_t data)
	{
...
struct Consumer2_1
{
	...
	uint64_t waitfor(Disruptor2_1 &dis)
	{
...
static Disruptor2_1 distruptor;
static Producer2_1 producer1;
static Consumer2_1 consumer1;
static std::atomic<bool> latch(false);

static void runConsumer(Disruptor2_1 &dis, Consumer2_1 &con, uint64_t thread_id)
{
...
static void runProducer(Disruptor2_1 &dis, Producer2_1 &pro, uint64_t thread_id)
{
